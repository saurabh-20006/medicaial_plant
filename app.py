...
def predict_image(model, image, class_names):
    ...
    avg_conf = np.mean(preds) * 100
    if confidence - avg_conf < 20:
...
if uploaded:
    ...
    normalized_leaf = leaf_label.lower().replace("_", "").replace(" ", "")
    normalized_plant = plant_label.lower().replace("_", "").replace(" ", "")

    st.subheader("✅ Final Prediction")

    if normalized_leaf == normalized_plant and leaf_label != "Uncertain" and plant_label != "Uncertain":
        final_label = plant_label
        final_conf = (leaf_conf + plant_conf) / 2
        st.success(f"🌿 Identified: **{final_label}** — {final_conf:.2f}% confidence")

    elif plant_conf > leaf_conf and plant_label != "Uncertain":
        final_label = plant_label
        final_conf = plant_conf
        st.success(f"🌳 Identified from Plant Model: **{final_label}** — {final_conf:.2f}% confidence")

    elif leaf_conf > plant_conf and leaf_label != "Uncertain":
        final_label = leaf_label
        final_conf = leaf_conf
        st.success(f"🍃 Identified from Leaf Model: **{final_label}** — {final_conf:.2f}% confidence")

    else:
        final_label = "Uncertain"
        st.warning("⚠️ The model is unsure. Try another image.")
